...
main = do file <- fmap (T.unpack) . filter (/= T.pack "") . T.split (== '\n') <$> IOT.readFile "input_day24.txt"
          ...
          print $ length . Map.keys . dayChange 100 $ Map.filter (\x -> x `rem` 2 == 1) tileMap
...
prepTiles tMap = prepTiles' (Map.keys tMap) tMap
           where prepTiles' [] tMap1 = tMap1
                 prepTiles' ((x,y):list) tMap1 = prepTiles' list $ tMap1 `Map.union` (Map.fromList $ zipWith (\x (Just y) -> (x,y)) forMap vList) 
                  where forMap =[(x+1,y),(x-1,y),(x+0.5,y+1),(x-0.5,y-1),(x+0.5,y-1),(x-0.5,y+1)]
                        vList = fmap (\x -> if Map.member x tMap1 then Map.lookup x tMap1 else Just 0) forMap 

travTiles tMap = travTiles' (Map.keys tMap) tMap 
      where travTiles' [] tMap1 = tMap1
            travTiles' ((x,y):list1) tMap1 | (vref `rem` 2 == 0) && nrBlack /= 2 || (vref `rem` 2 == 1) && (not $ nrBlack `elem` [1,2]) = travTiles' list1 $ Map.delete (x,y) tMap1
                                           | otherwise = travTiles' list1 $ Map.insertWith (+) (x,y) (fromEnum (vref `rem` 2 == 0)) tMap1          
                                                  where forMap = [(x+1,y),(x-1,y),(x+0.5,y+1),(x-0.5,y-1),(x+0.5,y-1),(x-0.5,y+1)]
                                                        vList = fmap (\x -> if Map.member x tMap then Map.lookup x tMap else Just 0) forMap 
                                                        Just vref = Map.lookup (x,y) tMap1
                                                        nrBlack = foldl' (\acc (Just x) -> acc + (x `rem` 2)) 0 vList
